...
从 schema 23 起，`user_reading_events` 保存用户本人对人类论文 sidecar 的追加式阅读确认历史，action 为 `confirmed | revoked | invalidated`。它与 Agent 的 `reading_status` / `critical_status` 完全分离。每次事件记录 `sidecar_path` 与归一化后的 Git content OID：计算前把顶部/结尾复选框都视为未勾选，并把 `<!-- akira:user-notes:start --> ... <!-- /akira:user-notes:end -->` 之间的用户专属笔记归一化为空区块。因而点击复选框或用户继续补写自己的笔记不会改变 Agent 阅读正文的版本身份；Agent 生成区域发生变化才会使旧确认不再适用于当前 content OID。残留的旧 `[x]` 会在同步时被 fail-closed 地标记 invalidated 并清空，不能替用户确认新版本。

当前人类 sidecar 使用 `<!-- akira:literature-note -->` 类型标记，不把格式版本号写进文件身份。completion 机械核验唯一一级标题、固定书目信息表、固定二级标题顺序、用户笔记边界、双确认框、文件命名以及 Literature README / Collection 导航，但不评价字数、实验数或图表数。旧项目迁移到该契约时记录迁移前 Git baseline；历史版本型 marker 只做确定性类型标记替换，更早的无 marker sidecar 仅在 baseline 中已存在且迁移后未修改时保留兼容，一旦编辑即必须采用当前固定格式。
...
Communication Product 的状态为 `draft | completed | superseded`。`completed` 可以在历史稿被新的 canonical 产品取代时转为 `superseded`，但这一 transition 不能同时改写 `title`、`purpose`、`audience`、`source_commit` 或 `canonical_source_path`；`superseded` 是终态，不能重新激活为 `draft` 或 `completed`。历史产品仍保留 artifact 与 provenance，但不再作为当前 completed product 参与活跃的 scientific source drift 或传播学术语言门禁。
