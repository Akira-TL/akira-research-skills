...
论文怎么读统一按 [`READING-PROTOCOL.md`](READING-PROTOCOL.md)；给人看的文件怎么组织、命名和跳转只按 [`references/HUMAN-FORMAT.md`](references/HUMAN-FORMAT.md)。`literature/` 是人类阅读区，不是 raw artifact 仓库：`literature/README.md` 是总入口，论文 Markdown 与可选同名 PDF 平铺在 `literature/papers/`，Collection 只放在 `literature/collections/`。文件名、固定 H1/H2、书目信息、双确认框、用户笔记边界、README/Collection 导航和历史迁移均由该格式 reference 定义，completion 机械校验；科学内容在 H3+ 按论文真实结构展开。

新生成或由 Agent 实质重写的论文 note 使用 `<!-- akira:literature-note -->` 类型标记，不把格式版本号写进文件身份；历史由 Git 管理。用户确认和 Agent Reconstruction / Critical Audit 继续完全分离：`research-db sync-user-reading` 只同步用户真实勾选，`akira:user-notes` 内已有用户内容由 Agent 逐字保留，Agent 正文变化不能继承旧确认。数据库保存完整结构化 extraction，sidecar 只保存高价值、可快速恢复理解的人类综合。
